...
if not ALLOWED_ORIGINS or ALLOWED_ORIGINS == [""]:
    ALLOWED_ORIGINS = [
        "https://email-ingestion-test.preview.emergentagent.com",
        "http://localhost:3000",
        "http://127.0.0.1:3000"
    ]
...
async def gmail_callback(code: str = None, state: str = None, error: str = None):
    ...
    frontend_url = "https://email-ingestion-test.preview.emergentagent.com"
...
async def send_csat_survey(
    ...
    base_url = "https://email-ingestion-test.preview.emergentagent.com"
...
def _resolve_field_type(field_name: str) -> str:
    """Resolve the type for a field, checking base fields then custom fields."""
    if field_name in FIELD_TYPE_MAP:
        return FIELD_TYPE_MAP[field_name]
    # Check custom fields (stored as custom_fields.<field_id>)
...
def _condition_to_mongo(condition: dict) -> dict:
    """Convert a single filter condition to a MongoDB query fragment."""
    field = condition["field"]
    op = condition["op"]
    value = condition.get("value")

    # Map the DB field path
    db_field = field
    if field.startswith("custom_fields."):
        db_field = field  # stored directly as custom_fields.<id>
...
def filter_tree_to_mongo(filter_tree: dict) -> dict:
    """
    Recursively convert a filter tree (with AND/OR groups and conditions)
    into a MongoDB query.
    
    Filter tree format:
    {
        "logic": "and" | "or",
        "conditions": [  // direct conditions in this group
            {"field": "status", "op": "is", "value": "todo"},
        ],
        "groups": [  // nested groups (brackets)
            {"logic": "or", "conditions": [...], "groups": [...]}
        ]
    }
    """
    ...
    # Process nested groups (brackets)
...
async def filter_tickets(
    ...
    """
    Advanced ticket filtering with AND/OR groups and nested conditions.
    Translates the filter tree to a MongoDB query for server-side filtering.
    """
...
async def get_filter_fields(current_user: dict = Depends(get_current_user)):
    ...
    and any custom fields defined by the admin.
    """
    base_fields = [
...
async def get_inboxes(current_user: dict = Depends(get_current_user)):
    """Get all custom inboxes visible to the current user (owned + shared)."""
    user_id = current_user["user_id"]
...
async def create_inbox(
    ...
    """Create a new custom inbox from a filter configuration."""
...
@app.put("/api/inboxes/{inbox_id}")
async def update_inbox(
    inbox_id: str,
    data: InboxUpdate,
    current_user: dict = Depends(get_current_user)
):
    """Update a custom inbox (name, filters, color)."""
    user_id = current_user["user_id"]
    inbox = custom_inboxes_collection.find_one({
        "inbox_id": inbox_id,
        "$or": [{"owner_id": user_id}, {"shared_with": user_id}]
...
@app.post("/api/inboxes/{inbox_id}/share")
async def share_inbox(
    ...
    """
    Share an inbox with other users. Creates a decoupled copy for each recipient.
    The recipient gets their own independent copy they can edit/delete.
    """
    inbox = custom_inboxes_collection.find_one({
        "inbox_id": inbox_id,
        "owner_id": current_user["user_id"]
...
@app.get("/api/inboxes/{inbox_id}/tickets")
async def get_inbox_tickets(
    ...
    """Fetch tickets matching an inbox's saved filter configuration."""
    user_id = current_user["user_id"]
    inbox = custom_inboxes_collection.find_one({
        "inbox_id": inbox_id,
        "$or": [{"owner_id": user_id}, {"shared_with": user_id}]
