...
export const Accordion = ({ children, className = '' }) => {
  ...
  if (items.length === 0 && children) {
    return (
      <div className={`my-6 border border-slate-800 rounded-lg divide-y divide-slate-800 ${className}`} data-testid="accordion">
        {children}
      </div>
    );
  }
  ...
  return (
    <div className={`${className}`} data-testid="accordion-item">
      ...
      <div className={`overflow-hidden transition-all duration-200 ${isOpen ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="px-4 pb-4 [&_p]:!text-slate-400 [&_code]:!text-emerald-400 [&_strong]:!text-slate-200 [&_a]:!text-[#188455]"
