...
import { 
  Calendar, Plus, ChevronLeft, ChevronRight, X, Check, 
  AlertTriangle, User, Clock, Trash2, Edit2
} from 'lucide-react';
...
const LeavePage = ({ user }) => {
  ...
  const checkConflicts = async (startDate, endDate) => {
    ...
    try {
      const response = await fetch(
        `${BACKEND_URL}/api/leaves/conflicts?start_date=${startDate}&end_date=${endDate}&exclude_user_id=${formData.user_id || user?.user_id}`,
        { credentials: 'include' }
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(error.detail || 'Failed to add leave');
      }
    } catch (error) {
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                      ...
                      {cell.leaves && cell.leaves.length > 0 && (
                        <div className="space-y-0.5">
                          {cell.leaves.slice(0, 3).map((leave, i) => (
                            <div
                              key={i}
                              className="text-[9px] px-1 py-0.5 rounded bg-primary/20 text-primary truncate"
          ...
          <div className="glass rounded-xl border border-border/60 p-4">
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto">
              {leaves
                .filter(l => l.status === 'approved' && l.start_date >= today.toISOString().split('T')[0])
                .slice(0, 10)
                .map(leave => (
                  <div
                    key={leave.id}
              ...
              {leaves.filter(l => l.status === 'approved' && l.start_date >= today.toISOString().split('T')[0]).length === 0 && (
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            <div className="bg-card rounded-2xl border border-border w-full max-w-md shadow-2xl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-border">
                <div>
                  <h3 className="font-semibold text-lg">Add Leave</h3>
                  <p className="text-sm text-muted-foreground">Record a new leave entry</p>
                </div>
                <button
                  onClick={() => setShowModal(false)}
