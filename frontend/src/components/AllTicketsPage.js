...
const AllTicketsPage = ({ user, onTicketClick }) => {
  ...
  const fetchTickets = async (pageNum) => {
    try {
      setLoading(true);
      // Fetch all open tickets (not resolved)
      ...
      // Filter out resolved tickets and sort by created_at (newest first)
      ...
      const paginatedTickets = openTickets.slice(startIdx, endIdx);
      ...
      setHasMore(endIdx < openTickets.length);
    } catch (error) {
      toast.error(error.message);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="px-6 py-4 border-b border-border/40">
        <h1 className="text-2xl font-semibold mb-1">All Open Tickets</h1>
        ...
        {tickets.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <div className="glass rounded-xl p-8">
              ...
              <p className="text-sm text-muted-foreground">All tickets are resolved!</p>
            </div>
          </div>
        ) : (
